...
export class ArchivesWorld {
  ...
  nearHide(){return (this.player.x>170&&this.player.x<360)||(this.player.x>1180&&this.player.x<1320)||(this.player.x>1570&&this.player.x<1740);}
  ...
  press(key:string){const k=key.toLowerCase();this.keys.add(k);if(k==='m')this.muted=!this.muted;if(k==='e'&&this.mode==='play'&&this.nearHide())this.player.hidden=!this.player.hidden;if(k==='t'&&this.mode==='play'&&this.nearTerminal()){this.note='TERMINAL ONLINE — enter the three-digit archive key.';this.hackInput='';}if(this.nearTerminal()&&['1','2','3','4','5','6','7','8','9','0'].includes(k)&&this.hackInput.length<3){this.hackInput+=k;if(this.hackInput.length===3){if(this.hackInput===this.hackSequence){this.hacked=true;this.note='ARCHIVE INDEX DECRYPTED — the last file is still intact.';}else{this.hackError=1.5;this.note='ACCESS DENIED — the terminal resets.';this.hackInput='';}}}}
  release(key:string){this.keys.delete(key.toLowerCase());}
  step(dt:number){if(this.mode!=='play')return;this.time+=dt;this.hackError=Math.max(0,this.hackError-dt);if(this.demo)this.runDemo();const left=this.keys.has('a')||this.keys.has('arrowleft');const right=this.keys.has('d')||this.keys.has('arrowright');const run=this.keys.has('shift');this.player.running=run;if(!this.player.hidden){const axis=(right?1:0)-(left?1:0);this.player.x+=axis*(run?185:110)*dt;if(axis)this.player.facing=axis;if((this.keys.has(' ')||this.keys.has('w')||this.keys.has('arrowup'))&&this.player.y>=this.floorY-this.playerHalfHeight)this.player.vy=-245;this.player.vy+=700*dt;this.player.y+=this.player.vy*dt;if(this.player.y>this.floorY-this.playerHalfHeight)this.player.y=this.floorY-this.playerHalfHeight,this.player.vy=0;}this.player.x=Math.max(35,Math.min(1740,this.player.x));const cameraTarget=Math.max(0,Math.min(840,this.player.x-360));this.cameraX+=(cameraTarget-this.cameraX)*Math.min(1,dt*8);
  ...
  private runDemo(){this.keys.clear();if(!this.hacked&&this.player.x<this.terminalX-20)this.keys.add('d');else if(!this.hacked){this.keys.add('t');for(const digit of this.hackSequence)this.keys.add(digit);}else{const target=this.pickups.find(p=>!p.taken)?.x??1730;if(this.player.x<target-8)this.keys.add('d');else if(this.player.x>target+8)this.keys.add('a');}}
}
