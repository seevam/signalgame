...
export class ArchivesWorld {
  ...
  press(key:string){const k=key.toLowerCase();if(k==='m'){this.muted=!this.muted;return;}if(this.mode!=='play')return;this.keys.add(k);if(k==='e'&&this.mode==='play'&&this.nearHide())this.player.hidden=!this.player.hidden;if(k==='t'&&this.mode==='play'&&this.nearTerminal()){this.note='TERMINAL ONLINE — enter the three-digit archive key.';this.hackInput='';}if(this.nearTerminal()&&['1','2','3','4','5','6','7','8','9','0'].includes(k)&&this.hackInput.length<3){this.hackInput+=k;if(this.hackInput.length===3){if(this.hackInput===this.hackSequence){this.hacked=true;this.note='ARCHIVE INDEX DECRYPTED — the last file is still intact.';}else{this.hackError=1.5;this.note='ACCESS DENIED — the terminal resets.';this.hackInput='';}}}}
