...
export function drawScene(c:CanvasRenderingContext2D,w:GameWorld,sprites:SpriteAssets={finn:null,guard:null,finnWalk:[],guardWalk:[],finnJump:null,finnLand:null,finnCrouch:null,finnHide:null,guardAlert:null,props:[]} ){
  ...
  const camX=690, camY=148, sweep=w.cameraSweep(), beam=w.cameraBeam(), camLive=w.interference<=0; c.save();c.globalAlpha=camLive?(w.cameraSeesPlayer()?.3:.15):.05;c.fillStyle=w.cameraSeesPlayer()&&camLive?C.alarm:C.glass;c.beginPath();c.moveTo(camX,camY+8);c.lineTo(beam.left,410);c.lineTo(beam.right,410);c.closePath();c.fill();c.restore();rect(c,camX-13,camY-4,26,12,'#252d49');rect(c,camX-7,camY+8,14,6,'#6474a2');rect(c,camX-3+Math.round(sweep*6),camY+10,6,4,C.alarm);txt(c,'CAM',camX-15,136,9,C.glass);
