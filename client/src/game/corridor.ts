...
export class CorridorWorld {
  ...
  press(key:string){const k=key.toLowerCase();if(k==='m'){this.muted=!this.muted;return;}if(this.mode!=='play')return;this.keys.add(k);if(k==='e'&&this.mode==='play'&&this.nearHide())this.player.hidden=!this.player.hidden;}
  ...
  private runDemo(){this.keys.clear();const target=this.pickups.find(p=>!p.taken)?.x??1930;if(this.player.x<target-8)this.keys.add('d');else if(this.player.x>target+8)this.keys.add('a');}
}
